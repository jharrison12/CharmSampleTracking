...
class ChildBioSpecimenEntryStool(FunctionalTest):
    ...
    def test_user_can_choose_status_of_child_stool_information_chooses_kit_sent_shipped_wsu(self):
        ...
        in_person_remote = Select(self.browser.find_element(By.ID, 'id_collected_child_stool_form-in_person_remote'))
        in_person_remote.select_by_visible_text('In Person')

        date_received = self.browser.find_element(By.ID, 'id_collected_child_stool_form-date_received')
        date_received.clear()
        date_received.send_keys('2023-09-27')

        number_of_tubes = self.browser.find_element(By.ID, 'id_collected_child_stool_form-number_of_tubes')
        number_of_tubes.send_keys(5)

        incentive_date = self.browser.find_element(By.ID, 'id_collected_child_stool_form-incentive_date')
